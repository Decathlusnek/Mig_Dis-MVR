...
/* 
 ...
 Modificações:
      ...
      13/04/2017: Alias temporário em todas as colunas da query interna usand o mesmo nome da coluna.
                  Substituição da função NVL para COALESCE.
      ...
      18/04/2017  Adição do Grupo Fornecedor. Esse dado é extraído do DW (db_link: @dsc_dw)            
*/
----------------------------------------------------------------------------------------------------------------------------------------------
...
SELECT qry.sup_name,
       ...
       /*qry.OBSERVACAO_NOTA AS E879454,*/
       ...
       /*,(qry.MENOR_DATA_PAGTO) AS E884621*/
       qry.total_discount_value,
       qry.schedule_date,
       qry.accounting_date,
       qry.last_update_datetime,
       qry.grupo_fornecedor_ebs,
       qry.vlr_icms_item_nf_complementar
       /*,qry.GRUPO_FORNECEDOR_EBS AS E1107610*/
FROM (SELECT --eb.vendor_name_alt grupo_fornecedor_ebs,
       prt.partner_id,
       prt.partner_desc,
       sup.supplier, 
       sup.sup_name,
       COALESCE (COALESCE((SELECT TO_CHAR(COALESCE(nsa.perc_vpc_exp,0),'000D00')||TO_CHAR(COALESCE(nsa.perc_ve_exp,0),'000D00')||TO_CHAR(COALESCE(nsa.perc_eq_exp,0),'000D00') perc_vpc_exp
       ...
       frn.grupo AS grupo_fornecedor_ebs,
       /* nb_re_orms_discoverer.f_obervacao_nota@orms(fdh.fiscal_doc_id)  observacao_nota, 'FAVOR PESQUISAR ESSA INFORMAÇÃO DIRETAMENTE NO RETAIL' */                      
    /*   (SELECT min(to_date(a.due_date,'yyyymmdd'))
          FROM rdw12dm.nb_d_ebs_financ_inf_dm@ordw a 
         WHERE a.invoice_source = 'RETAIL - ORFM' 
           AND a.vendor_site_id = TO_CHAR(su.supplier)
           AND a.invoice_nbr = TO_CHAR(fdh.fiscal_doc_no))           */
       /*(SELECT MIN(ac.due_date)
         FROM ap.ap_invoices_all@ebs_prod aa,
              ap.ap_payment_schedules_all@ebs_prod ac
        WHERE aa.source = 'RETAIL - ORFM'
          AND aa.invoice_id = ac.invoice_id
          AND aa.invoice_num = TO_CHAR(fdh.fiscal_doc_no)
          AND aa.vendor_site_id = su.supplier)*/--menor_data_pagto */
        (  SELECT SUM(fct.total_value)      
          FROM rmsprd.fm_fiscal_doc_complement fdc --d3,
          INNER JOIN rmsprd.fm_fiscal_doc_header fh ON fdc.compl_fiscal_doc_id = fh.fiscal_doc_id
          INNER JOIN rmsprd.fm_fiscal_doc_detail fdd ON fh.fiscal_doc_id = fdd.fiscal_doc_id
          INNER JOIN rmsprd.fm_fiscal_doc_tax_detail fct ON fdd.fiscal_doc_line_id = fct.fiscal_doc_line_id
          INNER JOIN rmsprd.fm_utilization_cfop fuc ON fh.utilization_cfop = fuc.utilization_cfop
          WHERE NOT EXISTS ( SELECT 1
                ...
                AND fh.schedule_no = fdh.schedule_no
       ...
       LEFT OUTER JOIN dw.dm_fornecedor@dsc_dw  frn ON (sup.supplier = frn.nt_cod_fornecedor_orms) OR (sup.supplier = frn.nt_cod_fornecedor_orms_cs)
       WHERE fsc.mode_type = 'ENT' 
             AND fdh.requisition_type = 'PO'
          /* AND ffu.requisition_type = 'PO' 
             AND ffu.mode_type = 'ENT' */
             AND fdh.location_type = 'W'
             AND fdh.module = 'SUPP'
             AND fdh.status not in ('E','I','W')
             AND fsc.status = 'C'
            -- AND su.supplier = eb.vendor_site_id(+)
        )qry
WHERE qry.ITEM IN ('220002')
      AND qry.ACCOUNTING_DATE >= TO_DATE('01/01/15','DD/MM,YY') AND qry.ACCOUNTING_DATE < TO_DATE('02/01/16','DD/MM/YY')--TRUNC(14/09/15)+1
      ...
      AND qry.ENTRY_OR_EXIT_DATE BETWEEN TO_DATE('20/07/15','DD/MM/YY') AND TO_DATE('17/12/17','DD/MM/YY')
      AND qry.ISSUE_DATE BETWEEN TO_DATE('13/07/15','DD/MM/YY') AND TO_DATE('17/12/17','DD/MM/YY')
/*ORDER BY qry.WH_NAME ASC, 
          qry.SUP_NAME ASC, 
          qry.ISSUE_DATE ASC, 
          qry.FISCAL_DOC_NO ASC*/ ;  
